...
void Task_Manage::menu(Assistant *Karen)
{
    ...
    while (sw_continue)
    {
        ...
        case Return:
            return;
        case See:
            Karen->speak(TASKS_MENU[See]);
            see_tasks(Karen);
            break;
        case Add:
            Karen->speak(TASKS_MENU[Add]);
            add_task();
            break;
        case Delete:
            Karen->speak(TASKS_MENU[Delete]);
            remove_task(Karen);
            break;
        case Complete:
            Karen->speak(TASKS_MENU[Complete]);
            complete_task(Karen);
            break;
        default:
            cout << Style::HEADLINE_SPACE;
...
void Task_Manage::complete_task(Assistant *Karen)
{
    string task{};
    update();
    system("CLS");
...
void Task_Manage::remove_task(Assistant *Karen)
{
    ...
    if (!days.at(day_location)->show_day(Karen))
        return;
    cout << Style::BORDER << Style::HEADLINE_SPACE;
...
void Task_Manage::see_tasks(Assistant *Karen)
{
    string day{};
    int day_location{};
    update();
...
/*---------- Destructor - deletes all days from days vector after program ends ----------*/
Task_Manage::~Task_Manage()
{
  for (auto day : days)
        delete day;
