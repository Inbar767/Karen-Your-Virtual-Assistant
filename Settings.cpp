...
namespace Settings_Functions
{
    ...
    void create_link(string &link, string &search)
    {
        stringstream hex;
    ...
    /*---------- insert_valid_choice - gets phrase to ask user for input, and integer limits. ----------
      ---------- Prints the phrase and gets from the user input, if it is not real number ----------
      ---------- or not in limit - return -1, else - return choice ----------*/
    int insert_valid_choice(const string &phrase, int low_limit, int high_limit)
    {
        string choice{};
        int number{};
        cout << phrase;
        ...
        if (!is_number(choice) || (number = stoi(choice)) < low_limit || number > high_limit)
            return -1;
        return number;
    }
